...
class Toolbox {
    ...
    switch (attr.service) {
      ...
      case "infura":
        if (!attr.key || !attr.secret) {
          throw new Error("INFURA projectId and projectSecret required");
        }
        execSync("npm install ndjson-parse", {
          stdio: [0, 1, 2],
        });
  ...
  async uploadCollectionNFT() {
    ...
    const response = await this.fileStorageService.uploadCollection(this.collection);
    return response;
  }
  ...
  async uploadSingleNFT(asset: string, metadata: any) {
    ...
    const response = await this.fileStorageService.uploadSingle(asset, metadata);
    return response;
  }
