  ...
  async generate() {
    ...
    while (indexCount <= this.schema.size) {
      ...
      if (isDnaUnique(dnaList, newDna)) {
        ...
        const loadedImages: Promise<canvas.Image>[] = selectedElements.map((element) => this.loadImage(element));
